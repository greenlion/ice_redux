...
#ifdef __WIN__  // WINPORT
#include "../../../../../src/storage/brighthouse/handler/ha_rcengine.h"
#else
#include "../storage/brighthouse/handler/ha_rcengine.h"
#endif
      ...
      if (!(res= open_and_lock_tables(thd, lex->query_tables)))
      {
        ...
        */
                                       ...
                                       select_tables)))
        {
          ...
          */
		  // Infobright hook added
		  int bh_res, free_join_from_bh, optimize_after_bh;
		  if(BH_HandleSelect(thd, lex, result, 0, bh_res, optimize_after_bh, free_join_from_bh, (int)true) == 0) {
			res= handle_select(thd, lex, result, 0, optimize_after_bh, free_join_from_bh);
		  } else {
			res= bh_res;
		  }
...
end_with_restore_list:
  ...
  */
  ...
  case SQLCOM_INSERT_SELECT:
  {
    ...
    if (!(res= open_and_lock_tables(thd, all_tables)))
    {
      ...
      if (!res && (sel_result= new select_insert(first_table,
                                                 ...
                                                 lex->ignore)))
      {
		  // BrightHouse tries to handle a query
		  // if BrightHouse leaves the query for MySQL engine
		  // then the derived tables are filled and the query gets into the original MySQL execution path
		  int bh_res, free_join_from_bh, optimize_after_bh;
		  if (BH_HandleSelect(thd, lex, sel_result, (ulong)OPTION_SETUP_TABLES_DONE, 
						bh_res, optimize_after_bh, free_join_from_bh, (int)true)==0)
			  res= handle_select(thd, lex, sel_result, OPTION_SETUP_TABLES_DONE, optimize_after_bh, free_join_from_bh);
      		  } else {
		  	  res= bh_res;
		  }
  ...
  case SQLCOM_SET_OPTION:
  {
	...
	if (!BH_SetStatementAllowed(thd, lex))
	{
		goto error;
	}
...
static bool execute_sqlcom_select(THD *thd, TABLE_LIST *all_tables)
{
  ...
  if (!(res= open_and_lock_tables(thd, all_tables)))
  {
    ...
    else
    {
     ...
     if (BH_HandleSelect(thd, lex, result, (ulong) 0, 
		bh_res, (int&)optimize_after_bh, free_join_from_bh) == 0) {
         res= handle_select(thd, lex, result, 0, optimize_after_bh, free_join_from_bh);
     }
     else
	 res= bh_res;
                 ...
                 const char ** found_semicolon)
{
  ...
  bool throttled = BH_ThrottleQuery(thd, inBuf, length);
