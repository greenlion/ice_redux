...
# Search the default brighthouse.ini file that comes with the installer.
for i in $basedir/support-files $basedir/share $basedir/share/mysql \
  $basedir/scripts `pwd` `pwd`/scripts @pkgdatadir@
do
  test -f $i/$bh_ini && bh_inipath=$i
done
test -f $ldata/$bh_ini      && mv -f ${ldata}/${bh_ini} ${ldata}/${bh_ini}_old
if test ! -f $bh_inipath/$bh_ini; then
  echo "CacheFolder = $cachedir" > $ldata/$bh_ini
else
  cp $bh_inipath/$bh_ini $ldata/;
  sed "s+@BH_CACHEDIR@+$cachedir+" $ldata/$bh_ini > $ldata/${bh_ini}.tmp
  mv $ldata/${bh_ini}.tmp $ldata/$bh_ini
fi
chmod 644 $ldata/$bh_ini;
${MYSQLD_BOOTSTRAP-$mysqld} --infobright-version > $ldata/$ib_ver
