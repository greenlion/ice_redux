...
int main(int argc,char *argv[])
{
  ...
  load_defaults("my-ib",load_default_groups,&argc,&argv);
  defaults_argv=argv;
  ...
  {
    /* read-history from file, default ~/.infobright_history*/
    if (getenv("INFOBRIGHT_HISTFILE"))
      histfile=my_strdup(getenv("INFOBRIGHT_HISTFILE"),MYF(MY_WME));
    else if (getenv("HOME"))
    {
      histfile=(char*) my_malloc((uint) strlen(getenv("HOME"))
				 + (uint) strlen("/.infobright_history")+2,
				 MYF(MY_WME));
      if (histfile)
	sprintf(histfile,"%s/.infobright_history",getenv("HOME"));
      char link_name[FN_REFLEN];
      if (my_readlink(link_name, histfile, 0) == 0 &&
          strncmp(link_name, "/dev/null", 10) == 0)
      {
        /* The .infobright_history file is a symlink to /dev/null, don't use it */
        my_free(histfile, MYF(MY_ALLOW_ZERO_PTR));
        histfile= 0;
      }
    }

    /* We used to suggest setting INFOBRIGHT_HISTFILE=/dev/null. */
...
static struct my_option my_long_options[] =
{
  ...
  {"port", 'P', "Port number to use for connection or 0 for default to, in "
   "order of preference, my-ib.cnf, $MYSQL_TCP_PORT, "
...
#include "sslopt-longopts.h"
  ...
  {"max_allowed_packet", OPT_MAX_ALLOWED_PACKET,
   ...
   (unsigned long long) 2*1024L*1024L*1024L, MALLOC_OVERHEAD, 1024, 0},
  ...
  {"select_limit", OPT_SELECT_LIMIT,
   ...
   (uchar**) &select_limit,
   (uchar**) &select_limit, 0, GET_ULONG, REQUIRED_ARG, 1000L, 1, LONG_MAX,
   0, 1, 0},
  {"max_join_size", OPT_MAX_JOIN_SIZE,
   "Automatic limit for rows in a join when using --safe-updates",
   (uchar**) &max_join_size,
   (uchar**) &max_join_size, 0, GET_ULONG, REQUIRED_ARG, 1000000L, 1, LONG_MAX,
   0, 1, 0},
...
and you are welcome to modify and redistribute it under the GPL license\n");
  ...
  print_defaults("my-ib", load_default_groups);
